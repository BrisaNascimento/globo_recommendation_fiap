...
acessos = download_from_adls('bronze', 'conformed/globo/users/users.csv') # splitted data
...
acessos['body_ajust'] = acessos['body'].astype(str).apply(lambda x: re.sub(r"[.,-]", " ", x))
...
def create_flag_lidos(df):
    ...
    df['flag_read'] = np.where(
        (df['timeOnPageHistory_sec'] >= df['min_req_time_sec']) & (df['scrollPercentageHistory_perc'] >= 0.50), 1, 0
    )
    return df

acessos = create_flag_lidos(acessos)
columns_to_drop = ['scrollPercentageHistory_perc', 'words_read', 'min_req_time_sec', 'timeOnPageHistory_sec', 'words_count']
acessos = drop_cols(acessos, columns_to_drop)
...
def filtro_leitores(df):
    user_counts = df.groupby('userId').size().reset_index(name='count')
    filtered_users = user_counts[user_counts['count'] >= 10]
    return df[df['userId'].isin(filtered_users['userId'])]

acessos_filt = filtro_leitores(acessos)[['userId', 'history', 'timestamp', 'flag_read']]

upload_to_dl(acessos_filt, 'silver', 'globo/acessos_filtrados/acessos_filtrados.parquet')

# CREATE ACESSOS_VAL VIEW

# Load and join aditional data
df_validacao = download_from_adls('bronze', 'conformed/globo/validation_cleanned/validation_cleanned.csv')
df_classes = download_from_adls('silver', 'globo/itens_text_db_scan/itens_text_db_scan.parquet')
...
# Join data
df_acessos_val = acessos.merge(df_validacao, left_on=['userId', 'history'], right_on = ['userId', 'history_adjusted'], how='left')
df_acessos_val = df_acessos_val.merge(df_classes, left_on='history', right_on='page', how='left')
# Remove unecessary columns
drop_columns = [
...
df_acessos_val = df_acessos_val.rename(columns={
                                                'userType_x': 'userType',
                                                'history_adjusted': 'history_val',
                                                'timestampHistory_adjusted': 'timestampHistory_val'
                                                })
