...
# Load Data from Lake
df_acessos = download_from_adls(
    'silver',
    'globo/acessos_flag_read/acessos.parquet'
    )
df_itens = download_from_adls(
    'bronze',
    'raw/globo/itens/itens.parquet')[['page', 'issued']]
df_noticias = download_from_adls(
    'silver',
    'globo/content_embbeding/content_embedding_cleaned.parquet')
...
df_noticias = df_noticias.merge(
    df_itens,
    left_on='page', right_on='page_2',
    how='inner'
    )
df_noticias = df_noticias[
    ['page', 'content_embbeding', 'issued', 'issued_date']
    ]

# Join access information to issued date
acessos = df_acessos.merge(
    df_noticias,
...
# Clean access and keeps only data accessed before news creation date
# For Consistency
...
min_last_entries = 5
acessos_cleanned.loc[:, 'rank'] = acessos_cleanned.groupby(
    'userId'
    )['timestamp'].rank(method='first', ascending=False)
ult_acessos = acessos_cleanned[acessos_cleanned['rank'] <= min_last_entries][
    ['userId', 'history', 'content_embbeding', 'timestamp', 'date']
    ]
...
noticias = df_noticias[
    df_noticias['issued_date'] >= pd.to_datetime('2022-08-13').date()
    ]
...
mais_acessadas = acessos_cleanned.groupby(
    'history'
    ).size().reset_index(name='access_count')
mais_acessadas = mais_acessadas.sort_values(by='access_count', ascending=False)
mais_acessadas['access_rank'] = mais_acessadas['access_count'].rank(
    method='first', ascending=False
    )
mais_acessadas = mais_acessadas.merge(
    noticias,
    left_on='history', right_on='page',
    how='inner')
mais_acessadas = mais_acessadas[
    mais_acessadas['issued_date'] >= pd.to_datetime('2022-08-13').date()
    ]
mais_acessadas['last_access_rank'] = mais_acessadas['access_count'].rank(
    method='first', ascending=False
    )
rank_top = 20
noticias_mais_acessadas = mais_acessadas[
    mais_acessadas['last_access_rank'] <= rank_top
    ]

noticias_mais_acessadas = noticias_mais_acessadas[
    ['history', 'last_access_rank']
]
noticias_mais_acessadas['last_access_rank'] = noticias_mais_acessadas[
    'last_access_rank'
    ].astype(int)

upload_to_dl(
    noticias_mais_acessadas,
    'silver', 'globo/noticias_mais_acessadas/mais_acessadas.parquet'
    )
